...
exports.createOrder = async (req, res) => {
  ...
  try {
    ...
    const orderResult = await client.query(
      `INSERT INTO orders (user_id, restaurant_id, total, items, status, created_at) 
       VALUES ($1, $2, $3, $4, $5, $6) 
       RETURNING *`,
    ...
    // Actualizar inventario (un query por cada ítem)
    for (let item of items) {
      const result = await client.query(
        `UPDATE inventory 
         SET quantity = quantity - $1 
         WHERE restaurant_id = $2 AND item_id = $3 AND quantity >= $1`,
    ...
    // Actualizar orden con info del pago
    await client.query(
      `UPDATE orders 
    ...
    // client.release();  <-- pendiente: evitar memory leak
